...
- Parameters
  - `gens`: A list of isometries $g_1,\dots, g_k\in SL(3,\mathbb{R})$ as `numpy.array` matrices.
  - `max_length`: The maximum word length of output words.
  - `center`: The center $X\in \mathcal{X}_3$ as a `numpy.array` matrix.
- Returns:
  - `wbs`: A list of `Word.Bis` elements consisting of bisectors $\mathrm{Bis}(X,g^{-1}.X)$ and isometry $g$ for all words $g$ in given generators up to the given length.
...
- Returns:
  - `wbs`: A list of bisectors with facet pairings bounding the new Dirichlet-Selberg domain.
  - `faces`: A list of faces describing the polyhedral structure of the new Dirichlet-Selberg domain.
...
- Parameters:
  - `wbs`: A list of bisectors with facet pairings bounding the Dirichlet-Selberg domain.
  - `faces`: A list of faces describing the polyhedral structure of the Dirichlet-Selberg domain.
  - `i_old`: The index of the first face in `my_face_list`.
  - `i_new`: The index of the second face in `my_face_list`.
  - `isom`: An isometry in $SL(3,\mathbb{R})$ as a `numpy.array` matrix.
- Returns:
...
- Parameters:
  - `wbs`: A list of bisectors with facet pairings bounding the Dirichlet-Selberg domain.
  - `faces`: A list of faces describing the polyhedral structure of the Dirichlet-Selberg domain.
  - `dest`: A point $X\in \mathcal{X}_3$ as a `numpy.array` matrix.
- Returns:
  - `path_word`: A list of indices $i_k$, such that the product of the isometries `my_wbs[i_k].word` takes $X$ into the given Dirichlet-Selberg domain.
